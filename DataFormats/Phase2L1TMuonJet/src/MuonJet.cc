...
bool MuonJet::isValidMuStub(const EMTFHitRef pStub)
{
  ...
  // in eta (1.2 - 1.6) must be hit from ME1/2
  //if(astubEta > 1.2 && astubEta < 1.6 && (stubStation != 1 || stubRing != 2 || stubType != EMTFHit::kCSC) ) rc = false;
  if(astubEta > 1.2 && astubEta < 1.5 && stubStation == 1 && stubRing == 2 && stubType == L1TMuon::kCSC ) rc = true;
  ...
  if(astubEta > 1.5 && astubEta < 2.4 && stubStation == 1 && (stubRing == 1 || stubRing == 4) &&  stubType == L1TMuon::kCSC)  rc = true;
  // in eta (2.0 - 2.8) must be hit from ME0
  //if(astubEta > 2.4 && astubEta < 2.8 && (stubStation != 1 || stubRing != 1 || stubType != EMTFHit::kME0) ) rc = false;
  if(astubEta > 2.4 && astubEta < 2.8 && stubStation == 1 && stubRing == 1 && stubType == L1TMuon::kME0)  rc = true;
...
bool MuonJet::isValidTkMuStub(int i)
{
  ...
  for (auto pStub : stubRefs_[i]) { 
    ...
    if(stubStation == 1 && stubType == L1TMuon::kCSC && astubEta > 2.4 ) rc = false;
...
float MuonJet::getPtFromBendingAngle(const EMTFHit & muStub) {
  ...
  if(type == L1TMuon::kCSC && station == 1) {
  ...
  if(type == L1TMuon::kME0 && station == 1) {
